...
export const withActionTracing =
  (handler: any) => (req: any, res: any, next: any) => {
    // trace/log here
    return handler(req, res, next);
  };

export const baseHandler = compose(withActionTracing);
...
export const allowedParams = (propertiesKey: any, { openapi }: any) => {
  const { allOf } =
    openapi.schema.requestBody.content['application/json'].schema.properties[
      propertiesKey
    ];
